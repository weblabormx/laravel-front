...
<td class="whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
    @if(!$helper->isDeleted())
        ...
        @endif
    @else
        @if( $helper->canRestore() )
            <!-- Restore button -->
            {!! \Front::buttons()->getByName('restore')->addLink($helper->restoreUrl())->setType('')->setTitle('')->setClass('inline-block text-primary-600 hover:text-primary-800')->form() !!}
        @endif
        @if( $helper->canForceDelete() )
            <!-- Force delete button -->
            {!! \Front::buttons()->getByName('forceDelete')->addLink($helper->forceDeleteUrl())->setType('')->setTitle('')->setClass('inline-block text-red-400 hover:text-red-600')->form() !!}
        @endif
    @endif
</td>
