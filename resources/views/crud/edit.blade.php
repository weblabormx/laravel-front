        ...
        {!! Form::model($object, array('method' => 'put', 'url' => $front->base_url.'/'.$object->getKey())) !!}
            ...
            <div class="text-right mt-3">
                @if( Auth::user()->can('delete', $object) )
                    <a data-type="confirm" title="{{ __('Do you really want to remove this item?') }}" data-info="{{ __('Do you really want to remove this item?') }}" data-button-yes="{{ __('Yes') }}" data-button-no="{{ __('No') }}" data-action="{{url($front->base_url.'/'.$object->getKey())}}" data-redirection="{{url($front->base_url)}}" data-variables='{ "_method": "delete", "_token": "{{ csrf_token() }}" }' class="btn btn-danger" href="#"><i class="fa fa-times pr-2"></i> {{ __('Delete') }}</a>
                @endif
