    ...
    <div
        x-data="{
            ...
            apply() {
                ...
                this.$wire.applyColumnPreferences(this.visible, this.manual);
                this.open = false;
            },
        }"
        ...
        aria-modal="true"
    >
        <div class="fixed inset-0 bg-secondary-950/30 backdrop-blur-sm" x-on:click="open = false" x-transition.opacity></div>
                ...
                <form x-on:submit.prevent="apply" class="w-full rounded-2xl bg-white shadow-xl ring-1 ring-secondary-900/10" x-transition>
                    <div class="border-b border-secondary-100 px-5 py-4 sm:px-6">
                        <div class="flex items-start justify-between gap-4">
                            ...
                            <button type="button" x-on:click="open = false" class="inline-flex h-8 w-8 items-center justify-center rounded-md text-secondary-400 hover:bg-secondary-100 hover:text-secondary-700">
                    ...
                    <div class="max-h-[65vh] overflow-y-auto px-5 py-4 sm:px-6">
                        <div class="space-y-2">
                            <template x-for="column in orderedColumns()" :key="column.key">
                                ...
                                >
                                    <button type="button" class="cursor-grab text-secondary-300 active:cursor-grabbing" x-bind:class="{ 'opacity-30': !isVisible(column.key) }" aria-label="{{ __('Drag to reorder') }}">
                                        <x-icon name="bars-3" class="h-4 w-4" />
                                    </button>

                                    <label class="flex min-w-0 flex-1 cursor-pointer items-center gap-3">
                                        <x-checkbox x-bind:checked="isVisible(column.key)" x-on:change="toggle(column.key)" />
                    ...
                    <div class="flex flex-col-reverse gap-3 border-t border-secondary-100 px-5 py-4 sm:flex-row sm:justify-between sm:px-6">
                        <button type="button" x-on:click="resetToDefault" class="inline-flex items-center justify-center rounded-md px-3 py-2 text-sm font-semibold text-secondary-500 hover:bg-secondary-100 hover:text-secondary-800">
                            {{ __('Reset') }}
                        </button>

                        <div class="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                            <button type="button" x-on:click="open = false" class="inline-flex items-center justify-center rounded-md px-4 py-2 text-sm font-semibold text-secondary-600 hover:bg-secondary-100">
                                {{ __('Cancel') }}
                            </button>
                            <button type="submit" x-bind:disabled="visible.length === 0" class="inline-flex items-center justify-center rounded-md bg-primary-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-primary-700 disabled:cursor-not-allowed disabled:opacity-50">
                                {{ __('Apply columns') }}
                            </button>
                        </div>
                    </div>
