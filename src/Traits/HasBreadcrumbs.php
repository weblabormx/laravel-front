...
trait HasBreadcrumbs
{
    ...
    public function getBreadcrumbsValues($object = null, $data = [])
    {
        ...
        // Index Action
        if ($front->source()->isCreate() && isset($data) && isset($data['action']) && !isset($object)) {
        ...
        // Action
        if ($front->source()->isCreate() && isset($data) && isset($data['action'])) {
            $breadcrumbs[] = ['title' => $front->plural_label, 'url' => $front->getIndexUrl()];
            if ($front->show_title) {
                $title = $front->title;
        ...
        // Massive
        if ($front->source()->isCreate() && isset($data) && isset($data['massive'])) {
            $breadcrumbs[] = ['title' => $front->plural_label, 'url' => $front->getIndexUrl()];
            if ($front->show_title) {
                $title = $front->title;
        ...
        // Index
        if ($front->source()->isindex()) {
            $breadcrumbs[] = ['title' => $front->plural_label, 'url' => $front->getIndexUrl()];
        }

        // Show normal
        if ($front->source()->isShow() && is_null($relation['front'])) {
            $breadcrumbs[] = ['title' => $front->plural_label, 'url' => $front->getIndexUrl()];
            $title = $this->title;
        ...
        // Create normal
        if ($front->source()->isCreate() && is_null($relation['front'])) {
            $breadcrumbs[] = ['title' => $front->plural_label, 'url' => $front->getIndexUrl()];
        ...
        // Edit normal
        if ($front->source()->isEdit() && is_null($relation['front'])) {
            $breadcrumbs[] = ['title' => $front->plural_label, 'url' => $front->getIndexUrl()];
            if ($front->show_title) {
                $title = $front->title;
